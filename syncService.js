...
(function () {
  ...
  // FIX: coleções que NUNCA colapsam — cada registro é único.
  // Colapsar saidas/financeiro fazia o SyncQueue sobrescrever registros
  // anteriores quando um novo era registrado antes do Firebase processar.
  // Resultado: o Firestore ficava só com o último array, perdendo os anteriores.
  const _NUNCA_COLAPSAR = new Set(['vendas', 'saidas', 'financeiro', 'movimentacoes', 'ponto']);

  function _colapsar(q, acao, colecao, dados) {
    if (_NUNCA_COLAPSAR.has(colecao)) return false;
